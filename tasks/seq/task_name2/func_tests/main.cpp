...
TEST(task_name2_seq, Test_Sum_10) {
  ...
  // Create Task
  TaskName2Sequential testTaskOpenMP(taskDataSeq);
...
TEST(task_name2_seq, Test_Sum_20) {
  std::vector<double> C(3 * 3, 0.0);
...
TEST(task_name2_seq, Test_Sum_50) {
  int n = 15, m = 15, k = 15;
  double ro = 0.3;
  std::vector<double> A(n * m, 0.0);
  TaskName2Sequential::genrateSparseMatrix(A.data(), A.size(), ro);
  std::vector<double> B(m * k, 0.0);
  TaskName2Sequential::genrateSparseMatrix(B.data(), B.size(), ro);
...
TEST(task_name2_seq, Test_Sum_70) {
  int n = 50, m = 50, k = 50;
  double ro = 0.3;
  std::vector<double> A(n * m, 0.0);
  TaskName2Sequential::genrateSparseMatrix(A.data(), A.size(), ro);
...
TEST(task_name2_seq, Test_Sum_100) {
  // int n = 5000, h = 600;
